# Titre Professionnel Developpeur Web et Web Mobile
Ce repo contient les rendus de mon Titre Professionnel Developpeur Web et Web Mobile.

<table style="border-collapse: collapse;">
    <thead>
        <tr>
            <th rowspan="2" style="border: 1px solid black;">Document</th>
            <th colspan="2" style="border: 1px solid black; text-align: center;">Dernière version</th>
        </tr>
        <tr>
            <th style="border: 1px solid black; text-align: center;">Date</th>
            <th style="border: 1px solid black; text-align: center;">Etat</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td style="border: 1px solid black;">Dossier projet</td>
            <td style="border: 1px solid black; text-align: center;">01/12/2025</td>
            <td style="border: 1px solid black; text-align: center;">dépôt final</td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">Dossier professionnel</td>
            <td style="border: 1px solid black; text-align: center;">12/01/2026</td>
            <td style="border: 1px solid black; text-align: center;"><em>dépôt examen *</em></td>
        </tr>
        <tr>
            <td style="border: 1px solid black;">Présentation d'examen</td>
            <td style="border: 1px solid black; text-align: center;"><em>à venir</em></td>
            <td style="border: 1px solid black; text-align: center;"><em>à venir</em></td> 
        </tr>
    </tbody>
</table>

<em>* Deux projets finalisés, deux projets en cours.</em>

Date de passage d'examen : vendredi 06 février 2026